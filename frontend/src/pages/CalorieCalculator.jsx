...
function CalorieCalculator() {
  ...
  useEffect(() => {
    const fetchMealData = async () => {
      try {
        // Simulated API response with enhanced nutritional data
        const response = {
          "Sri_Lankan_Foods": [
  ...
  const handleMealTimeChange = (e) => setSelectedMealTime(e.target.value);
  ...
  const addMeal = () => {
    ...
    for (const category of Object.keys(mealData)) {
      ...
      if (foundMeal) { mealData_item = foundMeal; break; }
    }
  ...
  // Calculate total nutritional values + warnings
  const calculateTotals = () => {
    let totalCal = 0; let totalSug = 0; let totalChol = 0;
  ...
  const formatMealName = (name) => name.replace(/_/g, ' ');

  // Helpers for progress UI
  const pct = (value, limit) => (value / limit) * 100;
  ...
  const calPct = pct(totalCalories, DAILY_LIMITS.calories);
  const sugPct = pct(totalSugar, DAILY_LIMITS.sugar);
  const cholPct = pct(totalCholesterol, DAILY_LIMITS.cholesterol);

  return (
    <div 
      className="flex justify-center items-center min-h-screen bg-gradient-to-r from-teal-100 to-blue-100 flex-col"
    >
              ...
              <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {categories.map(category => (
                        <option key={category} value={category}>{formatMealName(category)}</option>
                      ))}
                    </select>
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    >
                      <option value="" disabled>{selectedCategory ? 'Choose a meal' : 'Select a category first'}</option>
                      {selectedCategory && mealData[selectedCategory] &&
                        mealData[selectedCategory].map(item => (
                          <option key={item.name} value={item.name}>{formatMealName(item.name)}</option>
                        ))}
                    </select>
                  </div>
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 mb-1">
                      <span>Calories</span>
                      <span>{Math.round(calPct)}% of {DAILY_LIMITS.calories} kcal</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className={`${barColorClass(calPct)} h-2 rounded-full transition-all duration-300`} 
                        style={{ width: `${Math.min(calPct, 100)}%` }}
                      />
                    </div>
                    {calPct > 100 && (
                      <p className="mt-1 text-xs text-red-600"> ⚠️ You have eaten too much today. Be careful!</p>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 mb-1">
                      <span>Sugar</span>
                      <span>{Math.round(sugPct)}% of {DAILY_LIMITS.sugar}g limit</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className={`${barColorClass(sugPct)} h-2 rounded-full transition-all duration-300`} 
                        style={{ width: `${Math.min(sugPct, 100)}%` }}
                      />
                    </div>
                    {sugPct > 100 && (
                      <p className="mt-1 text-xs text-red-600">⚠️  You have eaten too much Sugar today. Be careful!.</p>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 mb-1">
                      ...
                      <span>{Math.round(cholPct)}% of {DAILY_LIMITS.cholesterol}mg limit</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        className={`${barColorClass(cholPct)} h-2 rounded-full transition-all duration-300`} 
                        style={{ width: `${Math.min(cholPct, 100)}%` }}
                      />
                    </div>
                    {cholPct > 100 && (
                      <p className="mt-1 text-xs text-red-600">⚠️ You have Exceeded daily cholesterol limit.Be careful</p>
                    )}
                  </div>
                </div>
              </div>
