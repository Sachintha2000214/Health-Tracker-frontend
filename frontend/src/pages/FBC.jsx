...
const FBC = () => {
  ...
  const handleManualSubmit = async (e) => {
    ...
    try {
      await axios.post("http://health-tracker-backend-s5ei.vercel.app/api/patient/fbc", {
        userId,
        docId,
        rbc,
        wbc,
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://health-tracker-backend-s5ei.vercel.app/api/patient/upload/fbc",
        formData,
        {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-teal-100 to-blue-100 flex-col">
      ...
      >
        ...
        <p className="text-center text-gray-500 text-sm mb-4 bg-white bg-opacity-70 rounded-md py-1">
          Date: {new Date().toLocaleDateString()}
        </p>

        {/* Tabs */}
        <div className="flex justify-between mt-6 bg-white bg-opacity-70 rounded-md p-1">
          <button
        ...
        {/* Tab Content */}
        <div className="tab-content mt-4 bg-white bg-opacity-70 rounded-md p-4">
          {activeTab === "manual" && (
            <form onSubmit={handleManualSubmit}>
              <div className="space-y-4">
                ...
                <button
                  type="submit"
                  className="mt-4 bg-teal-500 text-white py-2 rounded-md w-full"
                >
                  Save
                </button>
              </div>
